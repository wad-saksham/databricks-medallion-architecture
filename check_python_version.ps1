...
if ($pythonVersion -match "3\.13") {
    Write-Host ""
    Write-Host "⚠️  WARNING: Python 3.13 is not fully compatible with PySpark!" -ForegroundColor Red
...
if ($pythonVersions.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "📥 Please download Python 3.11 or 3.12 from:" -ForegroundColor Cyan
    ...
    foreach ($py in $pythonVersions) {
        $status = if ($py.Compatible) { "✅ COMPATIBLE" } else { "❌ NOT COMPATIBLE" }
